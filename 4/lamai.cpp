...
struct VMState {
    ...
    inline void push(Value v) {
        // Check for stack overflow already done
        *(__gc_stack_bottom++) = v.repr;
    }
    inline Value pop() {
        // Check for stack underflow already done
    ...
    inline void get_int_from_code_with_check(int32_t *v, char* code) {
    ...
    inline void execute_sexp() {
        ...
        aint *args = (aint*)__gc_stack_bottom - elem_count;
        push(tag_hash);
    ...
    inline void execute_ld_captured() {
        // std::cout << "LD C\n";
        int32_t addr;
        get_int_from_code_with_check(&addr, code);
        Frame *cf = get_current_frame();
    ...
    inline void execute_begin() {
        ...
        int32_t max_stack_size = (local_count >> 16) & 0xFFFF; // higher halfword is max required stack size
        local_count = local_count & 0xFFFF; // lower halfword is the actual local_count
        ...
        check(max_stack_size >= 0, "stack underflow. Offset: 0x%x\n", ip);
        check(stack_size() + max_stack_size < MAX_STACK_SIZE, "not enough space to execute code. Offset: 0x%x\n", ip);
    ...
    inline void execute_cbegin() {
        ...
        check(max_stack_size >= 0, "stack underflow. Offset: 0x%x\n", ip);
        check(stack_size() + max_stack_size < MAX_STACK_SIZE, "not enough space to execute code. Offset: 0x%x\n", ip);
        // Use info about maximum required stack size and adjust GC pointer accordingly
        __gc_stack_bottom = __gc_stack_bottom + local_count;
    ...
    inline void execute_closure() {
        ...
        aint *args = (aint*)(__gc_stack_bottom);
        push(target);
        for (int i = 0; i < n; i++) {
            int8_t type;
    ...
    inline void execute_make_array() {
        ...
        aint *args = (aint*)(__gc_stack_bottom - n);
